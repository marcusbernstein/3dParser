...
def log_time(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        timing_stats[func.__name__].append(elapsed_time)
        print(f"{func.__name__}: {elapsed_time:.4f} seconds")
        return result
    return wrapper
...
def calculate_cylinder_properties(vertices, face_indices, triangles):
    ...
    normals_dots = np.dot(face_normals, axis_vector)
    ...
    # ===== START OF CHANGE =====
    # Cache dot products for boundary point projections
    start_dot = np.dot(start_point - center_point, axis_vector)
    end_dot = np.dot(end_point - center_point, axis_vector)
    
    # Project boundary points using cached dot products
    start_proj = start_point - start_dot * axis_vector
    end_proj = end_point - end_dot * axis_vector
    # ===== END OF CHANGE =====
...
def detect_cylindrical_faces(vertices, triangles, angle_tolerance=0.001, area_tolerance=0.01, normal_tolerance=0.01, max_angle=np.pi/12):
    ...
    # ===== START OF CHANGE =====
    # Process only the filtered groups (instead of all updated_area_groups)
    for area, group_list in filtered_area_groups.items():
    # ===== END OF CHANGE =====
        # The original "if len(group_list) < 3: continue" check can be removed since we filtered above
        # But we'll keep it for safety in this conservative implementation
        if len(group_list) < 3:
            continue
...
def find_cylinder_boundaries(vertices, triangles, face_indices, axis, center):
    ...
    # ===== START OF CHANGE =====
    # Pre-compute edge-to-face mapping (once)
    edge_to_faces = defaultdict(list)
    for i, (tv1, tv2, tv3, _) in enumerate(triangles):
        for edge in [tuple(sorted([tv1, tv2])), tuple(sorted([tv2, tv3])), tuple(sorted([tv3, tv1]))]:
    ...
    for idx in face_indices:
       ...
       for edge in [(v1,v2), (v2,v3), (v3,v1)]:
           ...
           # ===== START OF CHANGE =====
           # Use pre-computed mapping instead of scanning all triangles
           connecting_faces = edge_to_faces[edge]
           # ===== END OF CHANGE =====
